...
export default function Dashboard() {
  ...
  const refresh = useCallback(async () => {
    if (role === ROLES.ioAdmin) {
      ...
      setStats([
        { label: "Programmes", value: progs.ok ? progs.data.length : 0 },
        { label: "Project requests", value: reqs.ok ? reqs.data.length : 0 },
        { label: "Projects to review", value: pending },
      ]);
    } else {
      const projs = await projectsRepository.as(actor).list();
      const mine = projs.ok ? projs.data : [];
      setStats([
        { label: "My submissions", value: mine.length },
        { label: "Approved", value: mine.filter((p) => p.reviewStatus === "approved").length },
        { label: "Pending review", value: mine.filter((p) => p.reviewStatus === "pending").length },
      ]);
    }
  }, [actor, role]);
  ...
  return (
    <div className="space-y-6">
      <Text variant="muted">Welcome back, {ACTOR_NAMES[actor.id] ?? actor.id}.</Text>
